...
std::string UdpSocket::host_interface() const
{
...
address UdpSocket::multicast_ip() const
{
...
void UdpSocket::join_multicast_group()
{
  udp_socket_.set_option(
    asio::ip::multicast::join_group(
      multicast_ip().to_v6(),
      if_nametoindex(host_interface().c_str())
    )
  );
  RCLCPP_INFO_STREAM(
    rclcpp::get_logger("UdpSocket::join_multicast_group"),
    "Joined multicast group " << multicast_ip() << " on interface " << host_interface() << ".");
}
