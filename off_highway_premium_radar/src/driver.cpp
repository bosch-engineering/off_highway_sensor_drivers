...
Driver::Driver(
  const std::string & host_interface, const int host_port, const std::string & multicast_ip,
  const std::string & sensor_ip, const int sensor_port,
  bool connect)
: ctx_(1),  // Use single additional thread to sequential process
  udp_socket_(ctx_, sensor_ip, sensor_port, host_interface, "::", host_port, multicast_ip)
{
  // Initialize UDP socket
  try {
    udp_socket_.open();
    udp_socket_.bind();
    udp_socket_.join_multicast_group();
    if (connect) {
