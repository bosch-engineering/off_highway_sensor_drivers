...
public:
  ...
  : ::testing::Test(),
    ctx_(1),
    udp_socket_(ctx_, multicast_ip_, host_port_, loopback_interface_, loopback_ip_, sensor_port_,
      multicast_ip_)
  {
    udp_socket_.open();
    udp_socket_.bind();
...
protected:
  void SetUp() override
  {
    ...
    std::vector<rclcpp::Parameter> params = {
      rclcpp::Parameter("host_interface", loopback_interface_),
      rclcpp::Parameter("host_port", host_port_),
      rclcpp::Parameter("multicast_ip", multicast_ip_),
...
private:
  const int sensor_port_ = 30406;
  const int host_port_ = 30405;
  const std::string loopback_interface_ = "lo";
  const std::string loopback_ip_ = "::1";
  const std::string multicast_ip_ = "FF14::4:4";
