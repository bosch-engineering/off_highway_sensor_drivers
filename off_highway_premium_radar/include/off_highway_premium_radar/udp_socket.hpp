...
public:
   ...
   * \param host_interface Host interface to join multicast group on
   * \param host_ip Host IP (empty to use any interface)
   * \param host_port Host port
   * \param multicast_ip Multicast IP
   */
  UdpSocket(
    const IoContext & ctx, const std::string & remote_ip, uint16_t remote_port,
    const std::string & host_interface, const std::string & host_ip, uint16_t host_port,
  ...
  std::string host_interface() const;
  std::string host_ip() const;
  uint16_t host_port() const;
  address multicast_ip() const;
...
private:
  ...
  //! Network interface to be used for joining the multicast group
  std::string host_interface_;
  //! For receiving multicast packets
  address multicast_ip_;
