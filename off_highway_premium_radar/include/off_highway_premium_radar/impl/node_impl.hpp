...
void Node<Converters...>::configure()
{
  ...
  driver_ =
    ...
    host_interface_, host_port_, multicast_ip_, sensor_ip_, sensor_port_, connect_socket_);
...
{
  host_interface_ = declare_parameter("host_interface", host_interface_);
  host_port_ = declare_parameter("host_port", host_port_);
  multicast_ip_ = declare_parameter("multicast_ip", multicast_ip_);
